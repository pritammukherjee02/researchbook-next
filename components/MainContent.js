...
function MainContent({ session }) {
    ...
    const articlesMarkup = articles.map((article) => {
        if (loading){
            ...
            return <ArticleCard key={article.id} articleId={article.data().articleId} title={article.data().title} thumbnailLink={article.data().thumbnailLink} description={article.data().description} uid={article.data().uid} author={article.data().author} date={article.data().date} />
        }
    })
